...
## Changelog
- July 7, 2025: **Fortune X Balance Validation Restored** - Restored Fortune X capital range validation with proper balance checking and clear error messages. System now checks live balance against duration requirements and shows messages like "Insufficient balance. In order to trade this duration you must have $20,000" when validation fails
- July 7, 2025: **Silent Trade Execution System** - Completely removed all notification messages from trade placement function and implemented silent validation based on Fortune X capital ranges. Trades now execute without any popup messages, with validation happening silently on both frontend and backend based on duration-specific capital requirements
...
- July 2, 2025: **Updated Balance Requirements and KYC Flow** - Restored KYC requirement for live trading with $50 welcome bonus after approval, updated balance requirements: 30s needs $49, 60s/90s need $60+, 120s/150s need $100+, other durations need $200+
- July 2, 2025: **Enhanced Insufficient Balance Messages** - Simplified balance validation error messages to exact format: "Insufficient balance. You have [current balance] in order to trade this duration you need [required amount]" showing correct amounts for each duration
...
- July 2, 2025: **CSRF Token Display Fix** - Fixed admin interface showing CSRF tokens in action buttons, now properly displays "Approve" and "Reject" buttons using hidden form fields for security tokens
- July 2, 2025: **Account Management Navigation** - Reorganized admin navbar with Account Management dropdown containing deposits, withdrawals, KYC verification, and user approvals with combined pending count badges
- July 2, 2025: **Admin User Approval System** - Implemented mandatory admin approval for new user registrations where users must wait for admin approval before accessing the platform, includes comprehensive admin interface with approval statistics and batch processing capabilities
- July 1, 2025: **Dashboard Chat Bot Widget** - Created simple floating chat widget on user dashboard for instant customer support communication, messages are stored as support tickets for admin review without complex navigation pages
